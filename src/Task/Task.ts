...
export class Task {
    ...
    public toggleCancel(): Task[] {
        const logger = logging.getLogger('tasks.Task');
        const codeLocation = 'toggleCancel()';
        logStartOfTaskEdit(logger, codeLocation, this);
        
        let newStatus = StatusRegistry.getInstance().getNextStatusOrCreate(this.status);
        if(this.status.type == StatusType.TODO && newStatus.type == StatusType.DONE) {
    ...
    public toggleCancelWithRecurrenceInUsersOrder(): Task[] {
        const newTasks = this.toggleCancel();
        return this.putRecurrenceInUsersOrder(newTasks);
    }
